CREATE DEFINER= 'sp_user'@'%' PROCEDURE `DAILY_TMS_REPORT_BY_DATE`(IN TRIP_DATE date, IN DCID int, IN ClientID int,
...
BEGIN
    select sum(if(t.delivery_center_id not in (58), (select count(*)
                                                     ...
                                                     where tdp1.trip_id = t.id
                                                       ...
                                                       and time(addtime(tdp1.check_in_time, IST_Diff)) <= TIME(
                                                             date_add(dp1.end_allowed_entry_time, interval AllowDelayMin minute))),
