CREATE DEFINER = `root` @`localhost` PROCEDURE `OverAll_VehiclePlan_Summary`(in ISTDIFF time) begin DECLARE TACE int;
DECLARE BOLERO INT;
DECLARE T407 INT;
select 
  date(
...
where 
  ...
  and t.vehicle_type = dcv.vehicle_type_id -- and t.delivery_center_id=DCID
